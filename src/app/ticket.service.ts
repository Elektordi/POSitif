...
import { Config, Order, Preorder } from './types';
...
export class TicketService {
  ...
  print_preorder_history_ticket(preorder: Preorder, history: Order[]): boolean {
    var ticket = this.encoder.initialize();
    ticket.raw([ 0x1c, 0x2e ]); // Disable chinese characters on some printers
    ticket.codepage('cp858');
    ticket.align('center');
    ...
    ticket.size('normal');
    ticket.line($localize`Preorder account` + ":");
    ticket.size('small');
    ticket.line(preorder.display_name);
    ticket.line(`(${preorder.uid})`);
    ticket.newline();
    ticket.size('normal');
    ticket.line($localize`Initial credit` + `: ${preorder.max} €`);
    ticket.line($localize`Used credit` + `: ${preorder.used} €`);
    ticket.line($localize`Balance` + `: ${preorder.max - preorder.used} €`);
    ticket.newline();
    ticket.rule({ style: 'single' })

    history.forEach( (order: Order) => {
      const datetime = new Date(order.payment_timestamp!).toLocaleString();
      const refund = order.refund ? -1 : 1;
      ticket.line($localize`Date ${datetime}`);
      ticket.size('small');
